...
export function BuildShowcase() {
  ...
  const compactList = PROJECTS.filter((p) => p.id !== featured.id);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-5 flex flex-col gap-6">
            {compactList.map((project) => (
              <div
                key={project.id}
                className="p-6 rounded-[28px] bg-[var(--surface)] border border-[var(--hairline)] shadow-sm hover:-translate-y-1 hover:shadow-lg transition-all duration-300 flex flex-col justify-between"
              >
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <h4 className="font-medium text-lg text-[var(--ink)]">
                      {project.title}
                    </h4>
